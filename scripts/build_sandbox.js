...
if (!baseImage?.length) {
  console.warn(
    'No default image tag specified in grok-cli/packages/cli/package.json',
  );
}
...
console.log('packing grok-cli ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'grok-cli-*.tgz'), { force: true });
execSync(
  `npm pack -w grok-cli --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing @grok/grok-cli-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'grok-cli-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w @grok/grok-cli-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `grok-cli-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(corePackageDir, 'dist', `grok-cli-core-${packageVersion}.tgz`),
  0o755,
);
...
function buildImage(imageName, dockerfile) {
  ...
  const imageTag =
    process.env.GROK_SANDBOX_IMAGE_TAG || imageName.split(':')[1];
