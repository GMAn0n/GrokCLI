...
let geminiSandbox = process.env.GROK_SANDBOX;
...
if (!geminiSandbox) {
  ...
  geminiSandbox = process.env.GROK_SANDBOX;
}
...
if (['1', 'true'].includes(geminiSandbox)) {
  if (commandExists('docker')) {
    ...
    console.error(
      'ERROR: install docker or podman or specify command in GROK_SANDBOX',
    );
    process.exit(1);
  }
  ...
  if (commandExists(geminiSandbox)) {
    ...
    console.error(
      `ERROR: missing sandbox command '${geminiSandbox}' (from GROK_SANDBOX)`,
    );
    process.exit(1);
  }
} else {
