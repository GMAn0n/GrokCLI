...
function getSandboxCommand(sandbox) {
    ...
    const environmentConfiguredSandbox = process.env.GROK_SANDBOX?.toLowerCase().trim() ?? '';
    sandbox =
    ...
    if (typeof sandbox === 'string' && sandbox) {
        ...
        console.error(`ERROR: missing sandbox command '${sandbox}' (from GROK_SANDBOX)`);
        process.exit(1);
    }
    ...
    if (sandbox === true) {
        console.error('ERROR: GROK_SANDBOX is true but failed to determine command for sandbox; ' +
