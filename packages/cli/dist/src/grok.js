...
import { ApprovalMode, EditTool, ShellTool, WriteFileTool, sessionId, logUserPrompt, AuthType, } from '@grok/grok-cli-core';
...
async function validateNonInterActiveAuth(selectedAuthType, nonInteractiveConfig) {
    ...
    if (!selectedAuthType && !process.env.XAI_API_KEY) {
        console.error(`Please set an Auth method in your ${USER_SETTINGS_PATH} OR specify XAI_API_KEY env variable file before running`);
        process.exit(1);
    }
    selectedAuthType = selectedAuthType || AuthType.USE_GROK;
    if (selectedAuthType === AuthType.USE_GROK) {
        // No validation needed for Grok
    }
