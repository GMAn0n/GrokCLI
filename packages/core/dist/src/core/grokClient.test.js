...
describe('GrokClient', () => {
    ...
    it('should initialize with tools and handle tool calls', async () => {
        ...
        const result = await grokClient.generateContent([{ role: 'user', parts: [{ text: 'Read the file test.txt' }] }], {}, new AbortController().signal);
        // Verify that the API was called with tools
        expect(fetch).toHaveBeenCalledWith('https://api.x.ai/v1/chat/completions', expect.objectContaining({
            method: 'POST',
            headers: {
...
//# sourceMappingURL=grokClient.test.js.map